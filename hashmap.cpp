...
// UIC Cs251
//
// implementation for our hashmap structure
// functions that I wrote:
// put, get, keys, containsKey
//
...
void hashmap::put(int key, int value) {
  ...
  hashkey = hashkey % nBuckets;
  key_val_pair *bucket = this->buckets[hashkey];
  if (bucket == nullptr) {
...
vector<int> hashmap::keys() const {
  ...
  for (int i = 0; i < this->nBuckets; i++) {
    key_val_pair *bucket = this->buckets[i];
    while (bucket != nullptr) {
